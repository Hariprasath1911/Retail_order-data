...
def Retail_order():
    ...
    queries = {
        ...
        round(sum(sale_price)::numeric,2) as Total_sale, sum(profit) as 
        Total_profit,round((sum(profit)/ sum(sale_price))*100) as profit_margin,
        rank() over(order by round(sum(sale_price)::numeric,2) desc) from 
        ...
        total_discount,round(sum(sale_price)::numeric,2) as total_sale,
        round((sum(discount_price)::numeric/sum(sale_price)::numeric)* 100,2)
...
def query_page():
    ...
    queries = {
        ...
        "Most Profitable Category": """
            SELECT p.category, ROUND(SUM(o.profit)::numeric, 2) AS total_profit 
            FROM product_data p 
            JOIN order_data o ON p.product_id = o.product_id 
            GROUP BY p.category 
        ...
        """,
        "Annual Revenue": """
            SELECT order_year, ROUND(SUM(sale_price)::numeric, 2) AS Revenue_per_year 
            FROM order_data 
            GROUP BY order_year;
        """
    }
...
def My_Query():
    ...
    queries = {
        ...
        select o.state, p.product_id,p.category,round(sum(o.sale_price)::numeric,2) as sale_price
        ...
        select p.category,sum(o.order_id) as order_count,o.quantity,round(sum(o.sale_price)::numeric,2)
        ...
        round(sum(o.sale_price)::numeric,2) as Revenue, round(sum(o.profit)::numeric,2) as profit from 
        ...
        select segment, round(sum(profit)::numeric,2) as profit, rank() 
        ...
        select segment, round(sum(sale_price)::numeric,2) as total_revenue 
        from order_Data group by segment;
        """,
        "Calculate the total profit for all regions":"""
        select region, round(sum(profit)::numeric,2) as Total_profit from order_Data 
        ...
        select order_month, round(sum(sale_price)::numeric,2) as profit,
